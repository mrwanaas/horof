...
async function createRoom(code, judgeName) {
  ...
  const state = {
    ...
    status: 'lobby',
    judge: judgeName,
    judgeToken,                 // secret token stored in room
    judgeOnline: true,
  ...
  // Store token locally so judge can authenticate
  lsSet(`judgeToken_${code}`, judgeToken);
  return state;
}

// Verify the current user is the judge of this room
async function verifyJudge(code) {
  const storedToken = lsGet(`judgeToken_${code}`);
  if (!storedToken) return false;
  const snap = await _fb.get(dbRef(`${roomPath(code)}/judgeToken`));
  return snap.exists() && snap.val() === storedToken;
}

// Check if room already has a judge token (room was created)
async function roomHasJudge(code) {
  const snap = await _fb.get(dbRef(`${roomPath(code)}/judgeToken`));
  return snap.exists();
}
